...
func TryBuiltin(fields []string, stdout, stderr io.Writer, registeredCompleters map[string]string, table *jobs.JobTable) (handled bool, shouldExit bool) {
	if len(fields) == 0 {
		return false, false
	}
	...
	case "complete":
		...
		return true, false
	case "jobs":
		builtins.Jobs(stdout, table.ListForDisplay())
		return true, false
	default:
		return false, false
	}
